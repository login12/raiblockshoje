...
void ProtocolSpectator::login(const std::string& liveCastName, const std::string& password)
{
	...
	if (liveCasterProtocol->isLiveCaster()) {
		...
		if (name.empty()) {
			std::ostringstream ss;
			ss << "Viewer " << client->viewerId;
			client->viewerId++;
			name = ss.str();
		}

		lastTalk = OTSYS_TIME() - 10000;
		
		liveCasterProtocol->addSpectator(std::static_pointer_cast<ProtocolSpectator>(shared_from_this()));
		
		if (client) {
			std::ostringstream ss;
			ss << name << " has entered on your cast channel.";
...
void ProtocolSpectator::logout()
{
	...
	if (client && player) {
		...
		std::ostringstream ss;
		ss << name << " left your cast channel.";
		//player->sendTextMessage(MESSAGE_EVENT_ADVANCE, ss.str());
		// sendChannelMessage("", ss.str(), TALKTYPE_CHANNEL_Y, CHANNEL_CAST);
		player->sendChannelMessage("", ss.str(), TALKTYPE_CHANNEL_O, CHANNEL_CAST);
...
void ProtocolSpectator::parseSpectatorSay(NetworkMessage& msg)
{
	...
	if (static_cast<int64_t>(OTSYS_TIME()) - static_cast<int64_t>(lastTalk) <= static_cast<int64_t>(g_config.getNumber(ConfigManager::SPECTATOR_TALK_DELAY) * 1000)) {
		if (!adminSpectator) {
			std::ostringstream ss;
			ss << "You are talking too fast. One message every " << g_config.getNumber(ConfigManager::SPECTATOR_TALK_DELAY) << " seconds.";
			sendChannelMessage("", ss.str(), TALKTYPE_CHANNEL_Y, CHANNEL_CAST);
			return;
		}
	}
	...
	if (text.substr(0, 5) == "/nick" && text.length() > 6) {
		...
		if (int(text.find("&")) > 0 || int(text.find("¨")) > 0 || int(text.find("%")) > 0 || int(text.find("#")) > 0 || int(text.find("@")) > 0 || int(text.find("!")) > 0 || int(text.find("god")) > 0 || int(text.find("God")) > 0 || int(text.find("GOD")) > 0 || int(text.find("gm")) > 0 || int(text.find("Gm")) > 0 || int(text.find("GM")) > 0 || int(text.find("adm")) > 0 || int(text.find("Adm")) > 0 || int(text.find("ADM")) > 0 || int(text.find("administrator")) > 0 || int(text.find("Administrator")) > 0 || int(text.find("ADMINISTRATOR")) > 0 || int(text.find("Administrador")) > 0 || int(text.find("administrador")) > 0 || int(text.find("ADMINISTRADOR")) > 0 || int(text.find("*")) > 0 || int(text.find("cm")) > 0 || int(text.find("Cm")) > 0 || int(text.find("cM")) > 0 || int(text.find("CM")) > 0) {
